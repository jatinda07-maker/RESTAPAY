...
import { parseToastSalesCategoryTotals, parseToastSalesRows } from '../src/engine/ToastSalesEngine.js'
...
console.log(JSON.stringify({ categories, checks }, null, 2))
for (const [a, b] of [[checks.categoryAlcohol, checks.expectedAlcohol], [checks.savedRowsAlcohol, checks.expectedAlcohol], [checks.categoryFood, checks.expectedFood], [checks.savedRowsFood, checks.expectedFood], [checks.savedRowsOther, checks.expectedOther], [checks.savedRowsExcluded, checks.expectedExcluded]]) {
