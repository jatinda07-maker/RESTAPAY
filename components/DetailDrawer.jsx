...
function buildDrawerContent(title, { metrics, sales, invoices, expenses, payroll, vendors, employees }) {
  ...
  const map = {
    'Period P&L': ['Selected period profit and loss', [{title:'Period P&L',rows:[['Adjusted Net Sales','Restaurant sales',appMoney(metrics.salesTotal)],['Food + Alcohol Cost','Includes manager allocation',appMoney(-metrics.allocatedFoodAlcoholCost)],['Employee Labor','Excludes manager + waiter tip pass-through',appMoney(-metrics.operatingLabor)],['Operating Expenses',`${expenses.length} records`,appMoney(-metrics.expenseTotal)]],total:['Operating Profit / Loss',appMoney(metrics.operatingProfit)]}]],
    'Sales Report': ['Live sales for the selected period', [{title:'Sales Report',rows:salesRows,total:['Net Sales',appMoney(metrics.salesTotal)]}]],
    'Payroll Report': ['Payroll audit with P&L pass-through exclusions', [{title:'Payroll Report',rows:[['Gross Payroll / Payments',`${payroll.length} records`,appMoney(metrics.payrollTotal)],['Manager Payroll','Allocated into Food/Alcohol cost',appMoney(metrics.managerLabor)],['P&L Employee Labor','Manager and waiter tips excluded',appMoney(metrics.operatingLabor)],['Customer Tip Pass-Through','Excluded from restaurant labor',appMoney(metrics.netTipsPaid)]],total:['P&L Employee Labor',appMoney(metrics.operatingLabor)]}]],
    'Expense Report': ['Operating expenses in the selected period', [{title:'Expense Report',rows:[['Operating Expenses',`${expenses.length} records`,appMoney(metrics.expenseTotal)],['Cash Expenses','Cash payments',appMoney(metrics.cashExpenses)]],total:['Operating Expenses',appMoney(metrics.expenseTotal)]}]],
    ...
    'Cash Remaining': ['Cash collected less cash payroll, cash invoices and expenses, with physical closing balance reconciliation', [{title:'Cash Remaining',rows:[['Opening Cash','Prior reconciled closing balance',appMoney(metrics.openingCash||0)],['Cash Collected','Sales cash receipts',appMoney(metrics.cashSales)],['Cash Payroll','Cash payroll payments',appMoney(-metrics.cashPayroll)],['Cash Vendor Invoices','Cash-paid vendor invoices',appMoney(-metrics.cashInvoiceSpend)],['Cash Expenses','Cash operating expenses',appMoney(-metrics.cashExpenses)]],total:['Cash Remaining',appMoney(metrics.cashRemaining)]}]],
...
export default function DetailDrawer({ title, entries = [], onClose }) {
  ...
  const [cashLedger,cashLedgerCrud] = useCrudCollection('restapay-cash-ledger',[])
  ...
  const saveClosingCash = async () => {
    const amount=Number(closingCash); if(!Number.isFinite(amount)||amount<0)return notify('Enter the physical closing cash balance.','error')
    try{await cashLedgerCrud.add({date:drawerRange.to,entry_date:drawerRange.to,type:'closing_balance',entry_type:'closing_balance',amount,purpose:'Physical cash reconciliation',notes:closingNote});setClosingCash('');setClosingNote('');notify(`Closing cash set to ${appMoney2(amount)} for ${drawerRange.to}.`,'success')}catch(error){notify(error?.message||'Cash balance save failed.','error')}
  }
  ...
  return <div className="drawer-layer" role="presentation" onMouseDown={onClose}>
    <aside className={`detail-drawer drawer-${content.tone} ${expanded ? 'drawer-expanded' : ''}`} role="dialog" aria-modal="true" aria-label={`${title} details`} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="drawer-scroll">
        ...
        {activeTab === 'Overview' && title === 'Cash Remaining' && <section className="drawer-section cash-adjustment-section"><h3>Cash Balance Adjustment</h3><p>Set the actual physical closing cash for the selected period. This reconciles cash only and does not create an operating expense.</p>{metrics.reconciledClosingCash!==null&&metrics.reconciledClosingCash!==undefined&&<div className="drawer-row drawer-entry-row"><span><strong>Saved Closing Balance</strong><small>{drawerRange.to}</small></span><b>{appMoney2(metrics.reconciledClosingCash)}</b></div>}<div className="form-grid"><label>Closing Date<input type="date" value={drawerRange.to||''} readOnly/></label><label>Physical Closing Cash<input type="number" step="0.01" min="0" value={closingCash} onChange={e=>setClosingCash(e.target.value)} placeholder="203.00"/></label><label>Reason / Notes<input value={closingNote} onChange={e=>setClosingNote(e.target.value)} placeholder="Physical cash count"/></label></div><button type="button" className="drawer-inline-action" onClick={saveClosingCash}>Set Closing Balance</button></section>}
