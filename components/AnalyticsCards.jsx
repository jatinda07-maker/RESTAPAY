...
function SummaryRow({ label, value, meta }) {
  return <div className={`summary-row ${/tip pass-through/i.test(label) ? 'summary-row-excluded' : ''}`}><div className="summary-row-copy"><strong>{label}</strong>{meta ? <span>{meta}</span> : null}</div><b>{value}</b></div>
}
...
export function FoodLaborCard({onOpen,metrics}) { return <SummaryCard onOpen={onOpen} icon={UtensilsCrossed} title="Cost Breakdown" subtitle="Current period totals" tone="orange" footerLabel="Prime Cost" footerValue={appMoney(metrics.primeCostAmount)}>
  <SummaryRow label="Food Cost" value={appMoney(metrics.trueFoodCost ?? metrics.foodCost)} meta={`${appPercent(metrics.trueFoodCostPercent ?? metrics.foodCostPercent)} of food sales`}/><SummaryRow label="Alcohol Cost" value={appMoney(metrics.trueAlcoholCost ?? metrics.alcoholCost)} meta={`${appPercent(metrics.trueAlcoholCostPercent ?? metrics.alcoholCostPercent)} of alcohol sales`}/><SummaryRow label="Employer Labor Cost" value={appMoney(metrics.employerLabor ?? metrics.operatingLabor)} meta={`${appPercent(metrics.laborMixPercent)} of sales · employee tips excluded`}/><SummaryRow label="Tip Pass-Through" value={appMoney(metrics.netTipsPaid)} meta="Employee-owned tips excluded from Prime Cost"/>
</SummaryCard> }
export function WeeklyProfitCard({onOpen,metrics}) { return <SummaryCard onOpen={onOpen} icon={TrendingUp} title="Profit Summary" subtitle="Income and deductions" tone="green" footerLabel="Operating Profit" footerValue={appMoney(metrics.operatingProfit)}>
  <SummaryRow label="Gross Sales" value={appMoney(metrics.salesTotal)} meta="Before expenses"/><SummaryRow label="Cost of Goods" value={appMoney(-metrics.cogs)} meta="Food and alcohol"/><SummaryRow label="Employer Labor & Expenses" value={appMoney(-((metrics.employerLabor??metrics.operatingLabor)+metrics.expenseTotal))} meta="Employee tips excluded"/>
</SummaryCard> }
