...
export default function Payroll(){
  ...
  const [bankChecks, bankCheckCrud] = useCrudCollection('restapay-bank-checks', [])
  ...
  const saveManual = async () => {
    ...
    try {
      ...
      const isManager=/general manager|restaurant manager|store manager|\bmanager\b|management/i.test(String(record.job_type||'')) && !/assistant manager|assistant mgr|asst\.? manager|asistente manager/i.test(String(record.job_type||''))
      if (status==='paid' && isManager && /check/i.test(String(record.payment_method||record.method||''))) {
        const ref=String(record.check_number||'').trim(); const payee=record.employee_name||record.employee||'Manager'; const amount=Number(record.total_pay||record.total||0)
        const exists=(bankChecks||[]).some(row=>String(row.payroll_entry_id||'')===String(record.id) || (String(row.date||row.payment_date||'')===String(record.payment_date||savedDate||'') && String(row.payee||'').toLowerCase()===String(payee).toLowerCase() && Number(row.amount||0)===amount))
        if(!exists) await bankCheckCrud.add({payment_date:record.payment_date||savedDate,date:record.payment_date||savedDate,type:'Check',payment_type:'Check',payee,reference:ref,reference_number:ref,amount,status:'Pending',notes:'Manager payroll check',payroll_entry_id:record.id})
      }
