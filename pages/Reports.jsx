...
export default function Reports() {
  ...
  const combinedTipRows = useMemo(() => {
    const grouped = new Map()
    payroll.filter(r=>Number(r.credit_card_tips||r.original_tips||r.tips||0)>0).forEach(r=>{
      const name=r.employee_name||r.employee||'Unknown employee'; const key=String(r.employee_id||name).toLowerCase()
      const current=grouped.get(key)||{name,hours:0,tips:0,withheld:0,extra:0,total:0}
      const tips=Number(r.credit_card_tips||r.original_tips||r.tips||0), withheld=Number(r.tip_deduction||r.tips_withheld||0), extra=Number(r.extra_pay||0)
      current.hours+=Number(r.hours||0); current.tips+=tips; current.withheld+=withheld; current.extra+=extra; current.total+=tips-withheld+extra; grouped.set(key,current)
    })
  ...
  const weeklyReportSections = [
    ...
    { title:'Employees With Tips — Combined by Employee', total:combinedTipRows.reduce((s,r)=>s+r.total,0), headers:['Employee','Hours','Original Tips','Withheld','Tips After Withholding','Extra Pay','Total'], rows:combinedTipRows.map(r=>[r.name,r.hours.toFixed(1),appMoney2(r.tips),appMoney2(r.withheld),appMoney2(r.tips-r.withheld),appMoney2(r.extra),appMoney2(r.total)]) },
    ...
    { title:'Period Profit / Loss Analysis', total:metrics.operatingProfit, headers:['Metric','Amount'], rows:[['Adjusted Net Sales',appMoney2(metrics.salesTotal)],['Food + Alcohol Cost (includes Manager allocation)',appMoney2(metrics.allocatedFoodAlcoholCost)],['Employee Labor (excludes Manager + waiter tip pass-through)',appMoney2(metrics.operatingLabor)],['Operating Expenses',appMoney2(metrics.expenseTotal)],['Operating Profit / Loss',appMoney2(metrics.operatingProfit)]] },
  ]
  ...
  const reportPdfPayload = key => {
    const commonSummary = [
      ...
      { label:'P&L Employee Labor', value:appMoney2(metrics.operatingLabor) },
