...
export function WeeklyProfitCard({onOpen,metrics}) { return <SummaryCard onOpen={onOpen} icon={TrendingUp} title="Profit Summary" subtitle="Income and deductions" tone="green" footerLabel="Operating Profit" footerValue={appMoney(metrics.operatingProfit)}>
  <SummaryRow label="Gross Sales" value={appMoney(metrics.salesTotal)} meta="Before expenses"/><SummaryRow label="Cost of Goods" value={appMoney(-metrics.cogs)} meta="Food and alcohol"/><SummaryRow label="Payroll & Expenses" value={appMoney(-(metrics.payrollTotal+metrics.expenseTotal))} meta="Labor and operating"/>
</SummaryCard> }
