...
const configs = {
  'restapay-employees': {
    table: 'employees',
    fromDb: r => ({ ...r, name:text(r.name||r.employee_name)||'Unnamed employee', job:r.job_type||r.job||'Other', type:r.employee_type||r.type||'Hourly', method:r.payroll_type||r.method||'Cash', basePay:money(r.base_pay), status:r.active===false?'Inactive':'Active', is_active:r.active!==false }),
    toDb: r => ({ id:r.id||id(), name:text(r.name)||'Unnamed employee', employee_type:r.employee_type||r.type||'Regular', job_type:r.job_type||r.job||'Other', pay_type:r.pay_type||'Hourly', payroll_type:r.payroll_type||r.method||'Cash', default_check_number:text(r.default_check_number||r.check_number), base_pay:money(r.base_pay??r.basePay), extra_pay:money(r.extra_pay), extra_reason:text(r.extra_reason), active:(r.status||'Active')!=='Inactive'&&r.is_active!==false&&r.active!==false, phone:text(r.phone), email:text(r.email), notes:text(r.notes), created_at:r.created_at||now(), updated_at:now() })
  },
  'restapay-invoices': { table: 'invoices' },
  'restapay-vendors': {
    table: 'vendors',
    fromDb: r => ({ ...r, name:text(r.name||r.vendor_name)||'Unnamed vendor', is_active:r.active!==false, type:r.vendor_type||r.type||'Inventory Purchase', expenseType:r.expense_type||r.expenseType||'Other', status:r.active===false?'Inactive':'Active', logo_url:r.logo_url||'' }),
    toDb: r => ({ id:r.id||id(), name:text(r.name)||'Unnamed vendor', category:r.category||'Other', contact:text(r.contact), phone:text(r.phone), email:text(r.email), default_check_number:text(r.default_check_number||r.check_number), notes:text(r.notes), active:(r.status||'Active')!=='Inactive'&&r.is_active!==false&&r.active!==false, vendor_type:r.type||r.vendor_type||null, expense_type:r.expenseType||r.expense_type||null, website:r.website||null, logo_url:r.logo_url||null, created_at:r.created_at||now(), updated_at:now() })
  },
  'restapay-expenses': {
    table: 'expenses',
  ...
  'restapay-payroll': {
    ...
    fromDb:r=>({ ...r, employee:r.employee_name, date:r.payroll_date, base_pay:money(r.regular_pay), original_tips:money(r.original_tips ?? (money(r.tips_after_withheld)+money(r.tips_withheld))), tips_after_withholding:money(r.tips_after_withheld), payment_method:r.method||'Cash', final_pay:money(r.total), total_pay:money(r.total), week_start:r.week_start||'', week_end:r.week_end||'', payment_status:r.payment_status||'Ready' }),
    toDb:r=>({ id:r.id||id(), employee_id:r.employee_id||null, employee_name:text(r.employee_name||r.employee)||'Unknown employee', source:r.source||r.source_type||r.group_name||'Manual', pay_type:r.pay_type||'Hourly', method:r.payment_method||r.method||'Cash', check_number:text(r.check_number), payroll_date:r.payroll_date||r.date||new Date().toISOString().slice(0,10), hours:money(r.hours), regular_pay:money(r.regular_pay??r.base_pay), original_tips:money(r.original_tips), tips_after_withheld:money(r.tips_after_withholding??r.tips_after_withheld), tips_withheld:money(r.tips_withheld), extra_pay:money(r.extra_pay), extra_reason:text(r.extra_reason), total:money(r.total_pay??r.final_pay??r.total), group_id:r.group_id||null, group_name:text(r.group_name), week_start:r.week_start||null, week_end:r.week_end||null, payment_status:r.payment_status||null, payment_date:r.payment_date||null, ach_reference:text(r.ach_reference), payment_notes:text(r.payment_notes||r.notes), source_ids:Array.isArray(r.source_ids)?r.source_ids:[], rolled_up:Boolean(r.rolled_up), weekly_rollup:Boolean(r.weekly_rollup), created_at:r.created_at||now(), updated_at:now() })
  },
  'restapay.sales': {
    table:'sales_days',
