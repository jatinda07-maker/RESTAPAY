...
async function saveInvoices(rows){
  ...
  const ids=new Set(invoiceRows.map(r=>r.id)); const lines=(rows||[]).flatMap(r=>(r.lines||[]).map(line=>({id:line.id||id(),invoice_id:r.id,description:text(line.description||line.item_name),item_name:text(line.item_name||line.description),quantity:money(line.quantity??1),unit:text(line.sales_unit||line.purchase_unit||line.unit),item_number:text(line.item_number),brand:text(line.brand),package_size:text(line.package_size),pack_count:money(line.pack_count),unit_size_value:money(line.unit_size_value),unit_size_unit:text(line.unit_size_unit),normalized_unit:text(line.normalized_unit),normalized_unit_cost:money(line.normalized_unit_cost),unit_price:money(line.unit_price),gross_unit_price:money(line.gross_unit_price??line.unit_price),discount_percent:money(line.discount_percent),discount_amount:money(line.discount_amount),net_unit_price:money(line.net_unit_price||((money(line.quantity)>0)?money(line.line_total)/money(line.quantity):0)),line_total:money(line.line_total),category:line.category||r.category||'Other',created_at:line.created_at||now()}))).filter(l=>ids.has(l.invoice_id))
