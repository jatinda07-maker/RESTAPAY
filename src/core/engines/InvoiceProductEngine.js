...
const UNIT_ALIASES = {
  ...
  ct: 'each', count: 'each', ea: 'each', each: 'each', pc: 'each', pcs: 'each', cn: 'each', can: 'each', cans: 'each', bt: 'bottle', btl: 'bottle', bottle: 'bottle', bottles: 'bottle',
  cs: 'case', case: 'case', box: 'case', bag: 'case', pkg: 'case', pack: 'case', bu: 'bushel'
}

const PURCHASE_UNIT_ALIASES = { cs:'case', case:'case', cases:'case', bt:'bottle', btl:'bottle', bottle:'bottle', bottles:'bottle', ea:'each', each:'each', pc:'each', pcs:'each', pkg:'pack', pack:'pack', box:'box' }
function canonicalPurchaseUnit(value) { const key=String(value||'').toLowerCase().replace(/[^a-z]/g,''); return PURCHASE_UNIT_ALIASES[key] || key || '' }
...
export function parsePackageDetails(input = {}) {
  ...
  const rawPurchaseUnit = input.purchase_unit || input.sales_unit || input.unit || input.uom
  let purchaseUnit = canonicalPurchaseUnit(rawPurchaseUnit)
  if (!purchaseUnit && packCount > 1) purchaseUnit = 'case'
  if (!purchaseUnit) purchaseUnit = 'each'
