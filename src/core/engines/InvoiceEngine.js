...
export function normalizeInvoiceLine(line = {}, index = 0) {
  ...
  return {
    ...
    gross_unit_price: n(line.gross_unit_price ?? enriched.unit_price),
    discount_percent: n(line.discount_percent),
    discount_amount: n(line.discount_amount ?? line.discount_per_unit),
    net_unit_price: quantity > 0 ? Number((lineTotal / quantity).toFixed(4)) : n(line.net_unit_price),
...
export function comparePrices(history = []) {
  ...
  return [...groups.values()].map(rows => {
    ...
    return {
      ...
      comparable_quantity: comparableQuantity,
      previous_date: previous.date || previous.invoice_date || '',
      current_date: latest.date || latest.invoice_date || '',
      effective_date: latest.date || latest.invoice_date || '',
