...
export function useAccessControl(){
  ...
  useEffect(()=>{let active=true;const load=async()=>{if(!isSupabaseReady)return;try{const{data}=await supabase.auth.getSession();const user=data?.session?.user;if(!active||!user)return;setIdentity({email:user.email||'',userId:user.id||''});const{data:row,error}=await supabase.from('app_user_roles').select('role').eq('user_id',user.id).maybeSingle();if(!error&&row?.role&&localStorage.getItem('restapay-current-role')!=='manager'){setRole(row.role);localStorage.setItem('restapay-current-role',row.role)}}catch{}};load();const{data:sub}=isSupabaseReady?supabase.auth.onAuthStateChange(()=>load()):{data:null};return()=>{active=false;sub?.subscription?.unsubscribe?.()}},[])
