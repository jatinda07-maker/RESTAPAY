...
export default function InvoicesV2({data,setData}){
  ...
  return <div className="rv2-mock-page rv2-invoices-mock">
    <div className="rv2-mock-action-row"><UploadBar label="Upload Invoice" accept=".pdf,.png,.jpg,.jpeg,.csv,.xls,.xlsx" onChange={handleUpload}/><button className="rv2-primary-button" onClick={openAdd}><Icon name="plus" size={17}/> Add Invoice</button></div>
    ...
    </div><footer><button className="rv2-clear-button" type="button" onClick={closeModal}>Cancel</button><button className="rv2-primary-button" type="submit"><Icon name="save" size={16}/>{editingId?'Update Invoice':'Save Invoice'}</button></footer></form></section></div>}
  </div>
}
