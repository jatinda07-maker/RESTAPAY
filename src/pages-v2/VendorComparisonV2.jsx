...
export default function VendorComparisonV2({ data }) {
  ...
  return <div className="rv2-page rv2-entity-v2 rv2-vendor-comparison-v2"><div className="rv2-mock-page rv2-vendor-comparison-mock">
    ...
    <div className="rv2-compare-summary"><div className="tone-green"><strong>{vendorA || 'Vendor A'}</strong><small>Total Items</small><b>{comparison.filter(row => row.left).length}</b><small>Avg Price</small><b>{money(averageA)}</b></div><div className="tone-navy"><small>Better Price</small><strong>{better}</strong><b>{compared.length} items compared</b></div><div className="tone-teal"><strong>{vendorB || 'Vendor B'}</strong><small>Total Items</small><b>{comparison.filter(row => row.right).length}</b><small>Avg Price</small><b>{money(averageB)}</b></div></div>
