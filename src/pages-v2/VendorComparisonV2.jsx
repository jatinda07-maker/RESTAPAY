...
export default function VendorComparisonV2({ data }) {
  ...
  return <div className="rv2-page rv2-entity-v2 rv2-vendor-comparison-v2"><div className="rv2-mock-page rv2-vendor-comparison-mock">
    <section className="rv2-compare-controls"><label>Category<select value={category} onChange={e => {setCategory(e.target.value);setPage(1)}}><option value="all">All Categories</option>{categories.map(item => <option key={item}>{item}</option>)}</select></label><label>Item<div className="rv2-search-control"><Icon name="search" size={15}/><input value={search} onChange={e => {setSearch(e.target.value);setPage(1)}} placeholder="All Items"/></div></label><label>Unit<select disabled><option>All Units</option></select></label><button className="rv2-orange-button" onClick={() => setPage(1)}><Icon name="compare" size={16}/> Compare</button></section>
