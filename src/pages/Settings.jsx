...
export default function Settings(){
  const access=useAccessControl();const[drawer,setDrawer]=useState(null),[tab,setTab]=useState('Business'),[geminiStatus,setGeminiStatus]=useState('Not tested'),[testingGemini,setTestingGemini]=useState(false);const {notify}=useFeedback()
  ...
  const [employees]=useCrudCollection('restapay-employees',[])
  ...
  const laborJobs=[...new Set((employees||[]).map(row=>String(row.job_type||row.job||row.position||row.role||'').trim()).filter(Boolean))].sort(alphaSort)
  const inferredLaborGroup=job=>{const text=String(job||'').toLowerCase();if(/manager|management/.test(text))return 'Management';if(/waiter|waitress|server|bartender|barback|host|hostess|front house|foh/.test(text))return 'FOH';if(/kitchen|cook|chef|prep|dishwasher|dish washer|busser|bus boy|boh/.test(text))return 'Kitchen / BOH';return 'Excluded / Other'}
  const laborGroupFor=job=>laborClassification?.[String(job).toLowerCase()]||inferredLaborGroup(job)
  const setLaborGroup=(job,value)=>setLaborClassification(prev=>{const next={...(prev||{}),[String(job).toLowerCase()]:value};setTimeout(()=>window.dispatchEvent(new CustomEvent('restapay:labor-classification-change')),0);return next})
...
${targets.map(x=>x.name).join(', ')}`)?.trim();const target=targets.find(x=>sameClassification(x.name,targetName));if(!target)return targetName&&notify(`Choose an existing ${label}.`,'error');const usage=classificationUsage(kind,item.name);if(!confirm(`WARNING: Merge "${item.name}" into "${target.name}"? This will update ${usageText(usage)} and deactivate the old value.`))return;if(kind==='expense'){
  ...
  return <div className="records-page">
    ...
    <section className="records-workspace card-surface"><header className="records-header"><div><h2>Settings</h2><p>Manage business profile, defaults, security, integrations, and interface preferences</p></div><button className="primary-button" onClick={()=>notify("Settings saved locally.")}><Save size={17}/>Save Settings</button></header>
      <div className="payroll-tabs settings-tabs">{['Business','Classifications','Labor Classification','Payroll Defaults','Cost Allocation','Invoice Defaults','Integrations','Users & Security','Appearance'].map(item=><button key={item} className={tab===item?'active':''} onClick={()=>setTab(item)}>{item}</button>)}</div>
      <div className="settings-panel">
        ...
        {tab==='Labor Classification'&&<section className="classification-manager labor-classification-manager"><header><div><h3>Labor Classification</h3><p>Assign each employee job type once. This mapping drives Payroll, Prime Cost, Labor Mix, Reports, Food/Alcohol allocations and KPI entries.</p></div></header><div className="classification-list">{laborJobs.map(job=><div className="classification-row labor-classification-row" key={job}><div><strong>{job}</strong><small>{(employees||[]).filter(row=>String(row.job_type||row.job||row.position||row.role||'').trim()===job).length} employee record(s)</small></div><label>Labor Group<select value={laborGroupFor(job)} onChange={e=>setLaborGroup(job,e.target.value)}><option>FOH</option><option>Kitchen / BOH</option><option>Management</option><option>Excluded / Other</option></select></label></div>)}{!laborJobs.length&&<div className="settings-help">Add employee job types first; they will appear here automatically.</div>}</div></section>}
        ...
        {tab==='Users & Security'&&<div className="security-settings"><section className="classification-manager"><header><div><h3>Current Access Role</h3><p>Role enforcement applies to navigation and write actions. Signed-in Supabase users can be mapped through app_user_roles.</p></div></header><div className="form-grid"><label>Current Role<select value={access.role} onChange={e=>access.setCurrentRole(e.target.value)}><option value="admin">Owner / Admin</option><option value="manager">Manager</option><option value="viewer">View Only</option></select></label><label>Signed-in User<input readOnly value={access.identity.email||'No authenticated user — Admin fallback'} /></label></div></section><div className="settings-list"><div><ShieldCheck/><span><strong>Owner / Administrator</strong><small>Full access, approval queue, cash controls, payroll, users, settings and financial configuration.</small></span><button type="button" disabled>Full Access</button></div><div><Users/><span><strong>Manager</strong><small>Reports + Sales/Shift uploads + Invoice uploads. Existing invoice edits become Pending Admin Approval. No delete, payroll, cash, settings or financial configuration access.</small></span><button type="button" disabled>Restricted</button></div></div></div>}
