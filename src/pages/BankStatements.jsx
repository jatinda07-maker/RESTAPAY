...
function parseUnionState(text, data) {
  ...
  const addCheck = (dateValue, checkNumber, amountValue, confidence = 88) => {
    const date = toISODate(dateValue, year)
    const amount = amountNumber(amountValue)
    const check = String(checkNumber || '').trim()
    const checkKey = `${date}-${check}`
    const key = `${checkKey}-${amount}`
    if (!date || !check || !amount || seen.has(key) || seenChecks.has(checkKey)) return
    seen.add(key)
    seenChecks.add(checkKey)
    out.push({ id: createId('bankrow'), selected: true, date, checkNumber: check, payee: `Check ${check}`, amount, sourceType: 'paper_check', confidence })
  }

  for (const line of lines) {
    let m = line.match(/^(\d{1,2}\/\d{1,2})\s+CHECK\s+(\d{3,8})\s+(\d{6,})\s+-?\$?([\d,]+\.\d{2})\b/i)
    if (m) {
      addCheck(m[1], m[2], m[4], 88)
      continue
    }

    // Union State includes a Checks summary: Date Number Amount Reference. Use it
    // because it is cleaner than the scrambled activity table in some PDF parsers.
    m = line.match(/^(\d{1,2}\/\d{1,2})\s+(\d{3,8})\s*\*?\s+\$?([\d,]+\.\d{2})\s+(\d{6,})\b/i)
    if (m) {
      addCheck(m[1], m[2], m[3], 90)
      continue
    }
...
function finalizeRows(rows, data) {
  return rows.map(row => {
    ...
    const genericCheckPayee = /^check\s*#?\d+$/i.test(String(enriched.payee || '').trim()) || /^check payment$/i.test(String(enriched.payee || '').trim())
    const preservedPayee = genericCheckPayee && match?.payee ? match.payee : enriched.payee
    const preservedCategory = enriched.category === 'Needs Review' && match?.category ? match.category : enriched.category
    const matchStatus = match?.status || (preservedCategory === 'Needs Review' ? 'Needs Review' : 'New Item')
    const action = match?.status === 'Exact Match' ? 'link_only' : match ? 'review' : 'import_new'
    return {
      ...enriched,
      payee: preservedPayee,
      category: preservedCategory,
      existingMatch: match,
      matchStatus,
      selected: row.selected !== false,
      reconcileAction: row.reconcileAction || action,
      status: preservedCategory === 'Needs Review' ? 'Needs Review' : 'Ready'
    }
