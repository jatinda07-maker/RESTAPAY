...
export default function VendorComparison(){
 ...
 const bestMatches=comparisons.filter(x=>Number(x.vendor_count||0)>1);const savingsMatches=comparisons.filter(x=>Number(x.potential_savings||x.savings||0)>0);const cards=[['Compared Items',comparisons.length,'Normalized item histories','blue',Tag,comparisons.flatMap(r=>r.history||[])],['Best Vendor Matches',bestMatches.length,'Comparable multi-vendor items','green',Store,bestMatches.flatMap(r=>r.history||[])],['Potential Savings',appMoney2(savings),'For comparable current quantities','purple',TrendingDown,savingsMatches.flatMap(r=>r.history||[])],['Invoice Lines',history.length,'Memorized price records','orange',WalletCards,history]]
