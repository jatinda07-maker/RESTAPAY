...
function isPayPeriodSummary(row = {}) {
  if (row.source_toast_labor_id || row.business_date) return false
  if (row.summary_only === true) return true
  const start = String(row.period_start || '').slice(0, 10)
  const end = String(row.period_end || '').slice(0, 10)
  const source = [row.source_file, row.group_name, row.source].map(value => String(value || '').toLowerCase()).join(' ')
  return Boolean(start && end && (/payrollexport|payroll export|toast/.test(source) || start !== end))
}

function summaryFitsSelectedRange(row = {}, start = '', end = '') {
  const periodStart = String(row.period_start || '').slice(0, 10)
  const periodEnd = String(row.period_end || '').slice(0, 10)
  if (!periodStart || !periodEnd) return false
  return (!start || start <= periodStart) && (!end || end >= periodEnd)
}
...
export default function Payroll({ data, setData, setActive }) {
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter(entry => {
      ...
      if (isPayPeriodSummary(entry)) {
        // A Toast PayrollExport row is one total for its complete report period.
        // Never count that whole-period amount inside a smaller date selection.
        if (!summaryFitsSelectedRange(entry, dateStart, dateEnd)) return false
      } else if (!inSelectedRange(entryDate)) return false
  ...
  function savePreviewToPayroll() {
    ...
    setData(prev => {
      ...
      const rows = sourceRows.map(row => {
        ...
        return {
          ...
          summary_only: Boolean(row.summary_only), source_file: row.source_file || '',
    ...
    setStatus(summaryImport ? `Saved ${sourceRows.length} full-period summary rows. They will only appear when the selected date range includes the complete Toast report period. Use Refresh Toast Labor for actual dated line entries.` : `Saved ${sourceRows.length} imported payroll rows and added unmatched employees to the employee list`)
  }


  const filteredToastReviewRows = useMemo(() => {
    const query = employeeSearch.trim().toLowerCase()
    const selectedName = employeeFilter === 'all' ? '' : employeeFilter.replace(/^name:/, '')
    return toastReviewRows.filter(row => {
      const rowDate = String(row.business_date || row.pay_date || '').slice(0, 10)
      if (!inSelectedRange(rowDate)) return false
      if (selectedName && normalizeName(row.employee_name || row.raw_employee_name) !== selectedName && normalizeName(displayToastName(row.raw_employee_name)) !== selectedName) return false
      if (query) {
        const searchable = [row.employee_name, row.raw_employee_name, row.job_name, row.pay_type, row.payroll_type, row.check_number].join(' ').toLowerCase()
        if (!searchable.includes(query)) return false
      }
      return true
    })
  }, [toastReviewRows, employeeSearch, employeeFilter, dateStart, dateEnd])

  const toastTipsReviewRows = useMemo(() => filteredToastReviewRows.filter(row => {
  ...
  }), [filteredToastReviewRows])

  const toastKitchenReviewRows = useMemo(() => {
    const tipIds = new Set(toastTipsReviewRows.map(row => row.id))
    return filteredToastReviewRows.filter(row => !tipIds.has(row.id))
  }, [filteredToastReviewRows, toastTipsReviewRows])
