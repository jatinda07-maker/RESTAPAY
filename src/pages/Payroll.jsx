...
export default function Payroll({ data, setData, setActive }) {
  ...
  const [activeTab, setActiveTabState] = useState(() => { try { return sessionStorage.getItem('restapay_payroll_tab') || 'tips-review' } catch { return 'tips-review' } })
  const setActiveTab = next => { try { sessionStorage.setItem('restapay_payroll_tab', next) } catch {}; setActiveTabState(next) }
  const [selectedEntryIds, setSelectedEntryIds] = useState([])
  const [bulkEditorOpen, setBulkEditorOpen] = useState(false)
  const [bulkForm, setBulkForm] = useState({ payroll_type: '', approval_status: '', check_number: '', paid_date: '' })
  ...
  function applyBulkPayrollEdit() {
    if (!selectedEntryIds.length) return
    const hasChange = Object.values(bulkForm).some(value => String(value || '').trim() !== '')
    if (!hasChange) {
      setStatus('Choose at least one bulk field to update.')
      return
    }
    ...
    setData(prev => ({
      ...
      payrollEntries: (prev.payrollEntries || []).map(entry => {
        ...
        const next = { ...entry, updated_at: now }
        if (bulkForm.payroll_type) {
          next.payroll_type = bulkForm.payroll_type
          next.payment_method = bulkForm.payroll_type
          next.method = bulkForm.payroll_type
        }
        if (bulkForm.approval_status) next.approval_status = bulkForm.approval_status
        if (bulkForm.check_number !== '') next.check_number = bulkForm.check_number
        if (bulkForm.paid_date) next.paid_date = bulkForm.paid_date
        return next
      })
    }))
    setStatus(`Updated ${selectedEntryIds.length} selected payroll entries.`)
    setBulkEditorOpen(false)
    setBulkForm({ payroll_type: '', approval_status: '', check_number: '', paid_date: '' })
    setSelectedEntryIds([])
  }
  ...
  return <>
    <style>{`
      ...
      .payroll-bulk-editor { display:grid; grid-template-columns:minmax(220px,1fr) repeat(4,minmax(150px,.75fr)) auto auto; gap:10px; align-items:end; padding:12px 14px; background:#f8fbff; border-bottom:1px solid #dbe7f3; }
      .payroll-bulk-editor strong { align-self:center; }
      .payroll-bulk-editor label { display:grid; gap:4px; font-size:11px; font-weight:800; color:#64748b; }
      .payroll-bulk-editor input,.payroll-bulk-editor select { height:40px; border:1px solid #dbe3ef; border-radius:9px; background:#fff; padding:0 10px; }
      @media (max-width:1200px){ .payroll-bulk-editor{grid-template-columns:repeat(2,minmax(180px,1fr))} }
      ...
      {activeTab === 'all' && <div className="payroll-content-grid">
        <section className="payroll-main-panel">
          ...
          <div className="payroll-bulk-row">
            ...
            <button className="btn primary" disabled={!selectedEntryIds.length} onClick={()=>setBulkEditorOpen(open=>!open)}><Icon name="edit" /> Bulk Edit ({selectedEntryIds.length})</button>
          ...
          {bulkEditorOpen && <div className="payroll-bulk-editor">
            <strong>Edit {selectedEntryIds.length} selected payroll entries</strong>
            <label>Payment Method<select value={bulkForm.payroll_type} onChange={e=>setBulkForm(prev=>({...prev,payroll_type:e.target.value}))}><option value="">No change</option><option>Cash</option><option>Check</option><option>Credit</option><option>ACH</option></select></label>
            <label>Status<select value={bulkForm.approval_status} onChange={e=>setBulkForm(prev=>({...prev,approval_status:e.target.value}))}><option value="">No change</option><option>Pending</option><option>Approved</option><option>Paid</option><option>Void</option></select></label>
            <label>Check #<input value={bulkForm.check_number} onChange={e=>setBulkForm(prev=>({...prev,check_number:e.target.value}))} placeholder="Leave blank unless changing" /></label>
            <label>Paid Date<input type="date" value={bulkForm.paid_date} onChange={e=>setBulkForm(prev=>({...prev,paid_date:e.target.value}))} /></label>
            <button className="btn primary" onClick={applyBulkPayrollEdit}>Apply Changes</button>
            <button className="btn secondary" onClick={()=>setBulkEditorOpen(false)}>Cancel</button>
          </div>}
