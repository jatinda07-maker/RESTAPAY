...
export default function Reports() {
  ...
  const weeklyReportSections = [
    ...
    { title:'Employees With Tips', total:payroll.reduce((s,r)=>s+(Number(r.credit_card_tips??r.original_tips??r.tips??0)-Number(r.tip_deduction??r.tips_withheld??0)),0), headers:['Date','Employee','Original Tips','Withheld','Tips After Withholding','Extra Pay','Reason','Total'], rows:payroll.filter(r=>Number(r.credit_card_tips??r.original_tips??r.tips??0)>0).map(r=>{const tips=Number(r.credit_card_tips??r.original_tips??r.tips??0),withheld=Number(r.tip_deduction??r.tips_withheld??0),extra=Number(r.extra_pay||0);return [r.pay_date||r.date||'',r.employee_name||r.employee||'',appMoney2(tips),appMoney2(withheld),appMoney2(tips-withheld),appMoney2(extra),r.extra_reason||'',appMoney2(tips-withheld+extra)]}) },
