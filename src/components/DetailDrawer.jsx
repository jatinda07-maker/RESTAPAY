...
function buildDrawerContent(title, { metrics, sales, invoices, expenses, payroll, vendors, employees }, explicitEntries = []) {
  ...
  const primeRows = [['Direct Food Purchases',`${metrics.foodInvoiceCount} invoices`,appMoney(metrics.foodCost)],['Direct Alcohol Purchases',`${metrics.alcoholInvoiceCount} invoices`,appMoney(metrics.alcoholCost)],['Operating Labor',`${metrics.primeCostLabor?.operatingDays?.length||operatingRows.length} daily BOH/kitchen allocations`,appMoney(metrics.primeOperatingLabor ?? metrics.operatingLabor)],['Management Payroll',`${metrics.primeCostLabor?.managementDays?.length||managementRows.length} daily management allocations · included once`,appMoney((metrics.primeManagementPayroll ?? metrics.managementPayroll ?? 0))],['Tip Pass-Through','EXCLUDED · employee-owned net tips',appMoney(metrics.netTipsPaid)]]
  ...
  const map = {
    ...
    'Prime Cost': ['Direct food/alcohol purchases plus daily allocated BOH/management labor; weekly fixed pay is divided by 7; tips excluded', [{title:'Prime Cost',rows:primeRows,total:['Prime Cost',appMoney(metrics.primeCostAmount)]}]],
    ...
    'Manager / GM & Other Payroll': ['Management, front-of-house wages and other employer payroll; customer tips excluded', [{title:'Manager / GM & Other Payroll',rows:[['Management Payroll',`${managementRows.length} management records`,appMoney(metrics.managementPayroll||0)],['Front of House Wages',`${fohRows.length} FOH wage records`,appMoney(metrics.frontOfHousePayroll||0)],['Other / Review Wages',`${reviewRows.length} uncategorized wage records`,appMoney(metrics.reviewPayroll||0)],['Customer Tips','Shown separately on Tips Check card',appMoney(0)]],total:['Manager / GM & Other Payroll',appMoney((metrics.managementPayroll||0)+(metrics.frontOfHousePayroll||0)+(metrics.reviewPayroll||0))]}]],
    'Kitchen Payroll': ['Kitchen / BOH employer wages only; customer tips excluded', [{title:'Kitchen Payroll',rows:[['Kitchen / BOH Wages',`${operatingRows.length} payroll records`,appMoney(metrics.operatingLabor||0)],['Customer Tips','Excluded from wage payroll',appMoney(0)]],total:['Kitchen Payroll',appMoney(metrics.operatingLabor||0)]}]],
    'Tips Check - Tipped Waiters': ['Employee-owned net tips paid after withholding; not restaurant wage expense', [{title:'Tipped Waiter Checks',rows:[['Original Tips','Before withholding',appMoney(metrics.payrollSummary?.tipsEarned||0)],['Tips Withheld','Configured withholding amount',appMoney(-(metrics.payrollSummary?.tipsWithheld||0))],['Net Tips Paid',`${fohRows.length} front-of-house payroll records`,appMoney(metrics.netTipsPaid||0)]],total:['Tips Check',appMoney(metrics.netTipsPaid||0)]}]],
...
if (lowerLabel === 'cash expenses') { rows = collections.expenses.filter(r => expenseMethod(r) === 'cash'); kind = 'expense' }
  ...
  else if (/^manager \/ gm & other payroll$/i.test(label)) { rows = [...managementPayroll,...frontPayroll,...reviewPayroll].map(r=>({...r,_display_amount:employerPayrollAmount(r)})).filter(r=>r._display_amount>0); kind='payroll' }
  else if (/^kitchen payroll$/i.test(label)) { rows = operatingPayroll.map(r=>({...r,_display_amount:employerPayrollAmount(r)})).filter(r=>r._display_amount>0); kind='payroll' }
  else if (/^tips check - tipped waiters$/i.test(label)) { rows = frontPayroll.map(r=>({...r,_display_amount:Number(r.net_tips ?? r.tips_after_withholding ?? r.tips_after_withheld ?? r.credit_card_tips ?? r.tips ?? 0)||0})).filter(r=>r._display_amount>0); kind='payroll' }
...
const routeMap = {
  ...
  'Payroll Total':'/payroll','Manager / GM & Other Payroll':'/payroll','Kitchen Payroll':'/payroll','Tips Check - Tipped Waiters':'/payroll','Cash Payroll':'/payroll','Check Payroll':'/payroll','Total Hours':'/payroll',
