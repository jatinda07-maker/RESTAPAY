...
function buildDrawerContent(title, { metrics, sales, invoices, expenses, payroll, vendors, employees }) {
  ...
  const costRows = [['Food Cost',`${metrics.foodInvoiceCount} invoices`,appMoney(metrics.foodCost)],['Alcohol Cost',`${metrics.alcoholInvoiceCount} invoices`,appMoney(metrics.alcoholCost)],['Operating Labor',`${payroll.length} payroll records · tips excluded`,appMoney(metrics.operatingLabor)],['Tip Pass-Through','Employee-owned net tips · excluded',appMoney(metrics.netTipsPaid)]]
  const map = {
    ...
    'Prime Cost': ['Food, alcohol and operating labor compared with sales', [{title:'Prime Cost',rows:[['Food Cost',`${metrics.foodInvoiceCount} invoices`,appMoney(metrics.foodCost)],['Alcohol Cost',`${metrics.alcoholInvoiceCount} invoices`,appMoney(metrics.alcoholCost)],['Operating Labor',`${payroll.length} payroll records · tips excluded`,appMoney(metrics.operatingLabor)],['Tip Pass-Through','Employee-owned net tips · excluded from Prime Cost',appMoney(metrics.netTipsPaid)]],total:['Prime Cost',appMoney(metrics.primeCostAmount)]}]],
    'Labor Mix': ['Operating labor compared with sales', [{title:'Labor Mix',rows:[['Operating Labor',`${payroll.length} payroll records · tips excluded`,appMoney(metrics.operatingLabor)],['Tip Pass-Through','Employee-owned net tips · excluded',appMoney(metrics.netTipsPaid)],['Net Sales',`${sales.length} sales days`,appMoney(metrics.salesTotal)]],total:['Labor Mix',appPercent(metrics.laborMixPercent)]}]],
    'Operating Profit': ['Sales less food, alcohol, operating labor and expenses', [{title:'Operating Profit',rows:[['Net Sales',`${sales.length} sales days`,appMoney(metrics.salesTotal)],['Cost of Goods','Food and alcohol',appMoney(-metrics.cogs)],['Operating Labor',`${payroll.length} payroll records · tips excluded`,appMoney(-metrics.operatingLabor)],['Tip Pass-Through','Employee-owned net tips · not deducted',appMoney(metrics.netTipsPaid)],['Expenses',`${expenses.length} records`,appMoney(-metrics.expenseTotal)]],total:['Operating Profit',appMoney(metrics.operatingProfit)]}]],
