...
Deno.serve(async request => {
  ...
  try {
    ...
    const prompt = `You are an invoice extraction engine for a restaurant accounting app. Extract invoice data from this file/image/PDF. Return only valid JSON, no markdown. Shape: {"vendor_name":"","invoice_number":"","invoice_date":"YYYY-MM-DD or raw date","invoice_type":"Regular Invoice|Credit Memo|Rebate|Return Credit|Vendor Adjustment","category":"Food|Beverage|Beer|Liquor|Utilities|Insurance|Supplies|Maintenance|Other","sales_subtotal":0,"total_discount":0,"total_charges":0,"net_amount":0,"tax":0,"total":0,"freight":0,"discount":0,"lineItems":[{"description":"","item_number":"","brand":"","qty":0,"unit":"","purchase_unit":"Case|Bottle|Each|Pack|Box|","package_size":"","pack_count":0,"unit_size_value":0,"unit_size_unit":"","unit_price":0,"discount_percent":0,"discount_amount":0,"total":0,"category":""}]}. IMPORTANT: lineItems.total must be the PRINTED line Amount/extended amount after any line-level discount. Never subtract a printed line discount from that line total again. total_discount is ONLY the invoice-summary discount. The printed final total is authoritative. For US Foods, Performance Foodservice/PFG, Sysco, beverage, beer, wine, liquor and Alabama ABC invoices, distinguish the PURCHASE UNIT from the package size. purchase_unit must be Case, Bottle, Each, Pack, Box, or empty when unclear. A line sold as CS/case is Case even when package_size is 6/750 ML. A line sold as BT/BTL/bottle is Bottle. Extract pack/case details exactly when printed (examples: 2/20 LB, 4/10 LB, 6/750 ML, 12/750 ML, 24/12 OZ). Never invent pack size. If bottles-per-case is unclear, leave pack_count 0 so the app can flag Needs Review instead of guessing. Use numbers only for amounts. If a field is unclear, use empty string or 0. File name: ${fileName}`
