...
export default function KpiCard({ title, value, subtitle, icon, tone = 'blue', onClick, compact = false }) {
  ...
  return (
    <Element className={`ui-kpi-card ui-tone-${tone}${compact ? ' is-compact' : ''}`} {...props}>
      <span className="ui-kpi-icon" aria-hidden="true"><Icon name={icon} size={19} /></span>
      ...
      {onClick ? <span className="ui-kpi-chevron" aria-hidden="true"><Icon name="chevronRight" size={16} /></span> : null}
    </Element>
  )
}
